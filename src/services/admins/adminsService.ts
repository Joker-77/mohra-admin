...
class AdminsService {
  public async getAll(input: AdminPagedFilterRequest): Promise<PagedResultDto<AdminDto>> {
    let result = await http.get('api/services/app/Admin/GetAll', {
      params: {
        ...
        advancedSearchKeyword: input.keyword,
  ...
  public async switchAdminActivation(input: EntityDto) {
    let result = await http.put('api/services/app/Admin/SwitchActivation', input);
    return result.data;
  }
